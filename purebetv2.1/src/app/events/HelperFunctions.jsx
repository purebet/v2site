    ...
    <div className="text-sm text-white font-semibold p-3 flex items-center">
      ...
      {showLiquidity && (
        <div className="ml-auto bg-white bg-opacity-10 px-2 md:px-4 py-1 flex items-center gap-1 rounded-full">
          <button className="md:text-xs text-[10px] text-white">
            Show Liquidity
          </button>
...
const BTTS = ({ title, data, showLiquidity = false }) => {
  ...
  // Extract odds safely
  const yesOdds = data.side0[0]?.[0]?.toFixed(2) || "-";
  const noOdds = data.side1[0]?.[0]?.toFixed(2) || "-";
...
const FullTime = ({
  ...
  const handleBetSelection = (key, value) => {
    ...
    const betData = {
      odds: Array.isArray(value)
        ? value[0][0]
        : typeof value === "object"
        ? value.odds
        : value,
      liquidity: Array.isArray(value)
        ? value[0][1]
        : typeof value === "object"
        ? value.side0?.[0]?.[1]
        : 0,
  ...
  const getBetButton = (key, value) => {
    const odds = Array.isArray(value)
      ? value[0][0]
      : typeof value === "object"
      ? value.odds
      : value;
...
const ResultandBothTeamstoScore = ({
  title,
  data,
  showLiquidity = false,
  homeTeam,
  ...
  return (
    <div className="mb-4 rounded-lg overflow-hidden">
      <div className="text-sm text-white font-semibold p-3 flex items-center">
        <span className="text-[#7D7D7D] opacity-65 text-2xl">&bull;</span>
        <span className="ml-1 font-poppins text-[10px] md:text-sm opacity-65">
          {title}
        </span>
        {showLiquidity && (
          <div className="ml-auto bg-white bg-opacity-10 px-2 md:px-4 py-1 flex items-center gap-1 rounded-full">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="flex items-center justify-center">
          <div className="flex items-center justify-center bg-[#132C42]  text-xs h-7 rounded w-9">
            Yes
          </div>
        </div>
        {Object?.entries(yesData)?.map(([key, value]) => (
          <div
            key={key}
            className="bg-[#132C42] w-[94px] text-white p-2  text-center rounded mt-1"
          >
            {value}
          </div>
        ))}
        ...
        {Object?.entries(noData)?.map(([key, value]) => (
          <div
            key={key}
            className="bg-[#132C42] w-[94px] text-white p-2 rounded mt-1 text-center"
          >
            {value}
          </div>
        ))}
    ...
    <div className="text-white p-4 rounded-lg">
      ...
      <div className="text-sm text-white font-semibold p-3 flex items-center">
        <div className="flex items-center">
          ...
          <span className="ml-1 font-poppins opacity-65">
            {title} - {marketLine}
          </span>
        </div>
        {showLiquidity && (
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        {/* Over Row */}
        <div className="flex items-center justify-center">
          <div className="flex items-center justify-center bg-[#132C42] text-xs h-7 rounded w-12">
            Over
          </div>
        </div>
...
const BothTeamstoScoreandTotal = ({
  title,
  data,
  showLiquidity = false,
  eventDetails,
  ...
  return (
    <div className="text-white p-4 rounded-lg">
      ...
      <div className="text-sm text-white font-semibold p-3 flex items-center justify-between">
        <div className="flex items-center">
          <span className="text-gray-500 text-2xl mr-2">•</span>
          <span className="ml-1 font-poppins text-[10px] md:text-xs opacity-65">
            {title}
          </span>
        </div>
        {showLiquidity && (
        ...
        <div className="">
          ...
          <div className="grid grid-cols-4 gap-2  ">
            ...
            {fullTimeResults?.map((result) => (
              <div
                key={result}
                className="text-center w-[94px] text-xs font-medium "
              >
                {result}
              </div>
            ))}
            ...
            {halfTimeResults?.map((halfTime) => (
              <React.Fragment key={halfTime}>
                ...
                <div className="flex items-center justify-center bg-[#132C42] text-xs h-7 rounded w-[65px] ml-16">
                  {halfTime}
                </div>
                ...
                {fullTimeResults?.map((fullTime) => (
                  <div
                    key={`${halfTime}-${fullTime}`}
                    className="bg-[#132C42] text-white p-2 rounded text-center w-[94px] "
                  >
  ...
  const renderMarketGroup = (marketData) => {
    return (
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div></div>
        <div className="text-center w-[94px] text-xs">
          {eventDetails?.homeTeam}
        </div>
        <div className="text-center w-[94px] text-xs">
          {eventDetails?.awayTeam}
        </div>
  ...
  return (
    <div className="text-white p-4 rounded-lg">
      ...
      <div className="text-sm text-white font-semibold p-3 flex items-center">
        <div className="flex items-center">
          <span className="text-gray-500 text-2xl mr-2">•</span>
          <span className="ml-1 font-poppins text-[8px] md:text-xs opacity-65">
            {title}
          </span>
        </div>
        {showLiquidity && (
...
const CorrectScore = ({ data, showLiquidity = false, eventDetails }) => {
  ...
  const [maxVisibleScores] = useState(3);

  const processOdds = (marketData) => {
    if (
      !marketData ||
      !marketData.side0 ||
      !Array.isArray(marketData.side0) ||
      marketData.side0.length === 0
    ) {
      return "N/A";
    }
  ...
  const groupScores = (scores) => {
    ...
    return [homeWin.sort(sortFn), draw.sort(sortFn), awayWin.sort(sortFn)];
  };

  const renderScoreColumn = (scores) => {
    const displayedScores = isExpanded
      ? scores
      : scores.slice(0, maxVisibleScores);
    const hasMore = scores.length > maxVisibleScores;
    ...
    return (
      <div className="flex flex-col gap-2">
        {displayedScores.map((score) => (
          <div
            key={score}
            className="bg-[#132C42] w-[94px] text-white p-2 rounded text-center"
          >
  ...
  return (
    <div className="text-white p-4 rounded-lg">
      <div className="text-sm text-white font-semibold p-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="ml-2 bg-white bg-opacity-10 px-2 py-1 rounded-full flex items-center gap-1"
          >
      ...
      <div className="flex justify-center gap-12">
        <div>
          <div className="text-center flex justify-center items-center min-w-[94px] mb-2 text-xs">
            {eventDetails?.homeTeam}
          </div>
